...
export default function Projects() {
    ...
    return(
        <div id="projects">
                ...
                <h2>Here are some of my accomplishemts over the years</h2>
...
let project_para1 = (
    <div>
        <p> This is probably one of my most proud projects I've ever done (yet!). We did this projec for our Capstone project made in collaboration with <a href="https://www.linkedin.com/in/eunice-js-choi/" target="blank" rel="noopenter noreferrer"> Eunice Choi</a> and under the supervision of <a href="https://sophiaytian.com/" target="blank" rel="noopenter noreferrer"> Prof. Yuan Tian </a>. The main idea of the project was to make a code tracking tool for developers who use the <a href="https://segment.com/" target="_blank" rel="noopener noreferrer" aria-hidden="true">Segment</a>, a customer data platform. Segment is used by developers to see how their customers interact with their websites. As their website grows bigger, more Segment calls are added, which makes it harder to keep track of them, especially for developers. </p>
        
        <p> We use regular expressions (RegEx) to do a complex and smart search in the user's codebase for all Segment API calls, and collect them in a table inside the IDE. This table shows information like line number, file name and other meta data of the Segment call. From here, developers can double-click on the row which will open the document inside the IDE and highlight the line where this is present. Users can also sort, filter the rows in the table too. A cool feature of this table is that it has dynamic theme switching to automatically switches between light, dark and high-contrast themes depending on the user's IDE theme for better visual clarity. </p>

        <p> Another feature of the extension is that it allows users to reuse previously used API calls from the table. Overall this vastly improved the developer experience and has removed some of the tedious tasks tracking these API calls. </p>
...
let project_para3 = (
    <div>
        <p> This was a project I did with <a href="https://qmind.ca/" target="_blank" rel="noopener noreferrer"> QMIND</a>, an AI research club at my University. I was part of a team with a team leader. Our task was to simulate a virtual autonomous driving environment. Because of the COVID pandemic, our project was done in a virtual environment using Gazebo. These softwares were run on Linux (Ubuntu) using Oracle's VM VirtualBox.</p>

        <p> We used ROS2, an open-source framework for robotics, inside Gazebo to implement and control a virtual Turtlebot. To get the Turtlebot to detect the road and lane we utilized the onboard camera sensor and fed the image to a machine learning model through various image transformation.  To detect other cars on the road, we utilized the onboard LIDAR to detect objects in its vicinity. Combing these two techniques, we were able to have the car drive around a virtual track and adjust its speed based on the curve of the road and other Turtlebots present. </p>

        <p> We showcased our project at  <a href="https://cucai.ca/" target="_blank" rel="noopener noreferrer">CUCAI</a> in March 2021, where we discussed our project and  presented a demo of our autonomous highway. Our research paper was selected amongst the <a href="https://www.linkedin.com/posts/qumind_proceedings-of-cucai-2021-activity-6794786756949532672-NJdQ/" target="_blank" rel="noopener noreferrer"> top 5 best papers! </a> at CUCAI </p>
    </div>
);

let project_para4 = (
    <div>
         <p> This was a machine learning group project to classify images of recyclable and organic waste. We used Python, TensorFlow and Numpy for this project. We got our dataset from Kaggle, which contained 2513 images. From here, we were able to do the necessary preprocessing steps, like resizing and converting the images to a suitable format.</p>

        <p> We implemented a convolutional neural network with pooling layers to train the model. We used the ADAM optimizer for gradient descent. Random Zoom and Dropout Layers were added to prevent overfitting. Our model achieved 87% accuracy on the testing data. We compiled our results and discussed in detail in our final report. </p>
    </div>
);